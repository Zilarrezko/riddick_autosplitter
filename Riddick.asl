...
{	
	...
	vars.InjectionBase = game.AllocateMemory(0x1000); // Note: Allocate one page
	vars.IsLoadingBase = vars.InjectionBase + 0x30;
	vars.MapNameBase = vars.InjectionBase + 0x38;
	print("[The Chronicles of Riddick] Injection Code at: 0x" + vars.InjectionBase.ToString("X"));
	
	var CodeForInjection = new List<byte>()
	{
/*0x00*/0x89, 0x2D, /*IsLoadingBase*/          // Note: mov [IsLoadingBase], ebp
/*0x06*/0x83, 0x85, 0xB4,0x12,0x00,0x00, 0x01, // Note: add dword ptr [ebp+000012B4], 01
/*0x0D*/0x68, /*LoadFlagWriteCode*/            // Note: push [LoadFlagWriteCode] + 7
/*0x12*/0xC3,                                  // Note: ret
/*0x13*/0x8D, 0x56, 0x04,           // Note: lea edx, [esi+04]
/*0x16*/0x89, 0x15, /*MapNameBase*/ // Note: mov [MapNameBase], edx
/*0x1C*/0x8B, 0xC8,                 // Note: mov ecx, eax
/*0x1E*/0x29, 0xCA,                 // Note: sub edx, ecx
/*0x20*/0x68, /*MapNameWriteCode*/  // Note: push [MapNameWriteCode]
/*0x25*/0xC3,                       // Note: ret
/*0x26*/0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Note: Padding buffer for aligning MapNameBase and IsLoadingBase
/*0x30*/0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Note: This will be where IsLoadingBase pointer will be. Room for an 8 byte address
/*0x38*/0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00  // Note: This will be where MapNameBase pointer will be. Room for an 8 byte address
	};
	CodeForInjection.InsertRange(0x02, BitConverter.GetBytes((int)vars.IsLoadingBase));
	CodeForInjection.InsertRange(0x0E, BitConverter.GetBytes((int)vars.LoadFlagWriteCode + 7));
	CodeForInjection.InsertRange(0x18, BitConverter.GetBytes((int)vars.MapNameBase));
	CodeForInjection.InsertRange(0x21, BitConverter.GetBytes((int)vars.MapNameWriteCode + 7));
	
	var JumpToLoadingFlagInjection = new List<byte>()
	{
		0x68, /*InjectionBase*/ // Note: push [InjectionBase]
		0xC3,                   // Note: ret
		0x90                    // Note: nop
	};
	JumpToLoadingFlagInjection.InsertRange(1, BitConverter.GetBytes((int)vars.InjectionBase));
	
	var JumpToMapNameInjection = new List<byte>()
	{
		0x68, /*InjectionBase + 0x13*/ // Note: push [InjectionBase + 0x13]
		0xC3,                          // Note: ret
		0x90                           // Note: nop
	};
	JumpToMapNameInjection.InsertRange(1, BitConverter.GetBytes((int)vars.InjectionBase + 0x13));
	...
	game.WriteBytes((IntPtr)vars.InjectionBase, CodeForInjection.ToArray());
	game.Resume();
